...
if [ "`update-alternatives --list javac | wc -l`" -gt 1 ]; then
        JDK6=$(dirname `update-alternatives --list javac | grep "\-7\-"` | tail -n1)
        JRE6=$(dirname ${JDK7}/../jre/bin/java)
...
case "$EXTRAS" in
        threads)
                echo -e "Please enter desired building/syncing threads number followed by [ENTER]${txtrst}"
                read threads
                THREADS=$threads
        ;;
        clean|cclean)
                echo -e "Cleaning intermediates and output files${txtrst}"
...
if [ "$SYNC" == "true" ]; then
        echo -e ""
        echo -e "Fetching latest sources${txtrst}"
        repo sync -j"$THREADS"
        echo -e ""
fi

if [ ! -r "${DIR}/out/versions_checked.mk" ] && [ -n "$(java -version 2>&1 | grep -i openjdk)" ]; then
        echo -e "Your java version still not checked and is candidate to fail, masquerading.${txtrst}"
...
if [ -n "${INTERACTIVE}" ]; then
        ...
        # Setup environment
        echo -e ""
        echo -e "Setting up environment${txtrst}"
        . build/envsetup.sh
        echo -e ""
...
# Get elapsed time
res2=$(date +%s.%N)
echo -e "Total time elapsed: ${txtrst}$(echo "($res2 - $res1) / 60"|bc ) minutes ($(echo "$res2 - $res1"|bc ) seconds)${txtrst}"
